...
public class ExcelTools : EditorWindow
{
    ...
    /// 输出格式
    /// </summary>
    private static string[] formatOption = new string[] { "ScriptableObject", "Json", "Csv", "Xml" };
    ...
    private static void Convert()
    {
        foreach (string assetsPath in excelList)
        {
            ...
            if (indexOfFormat == 0)
            {
                excel.GenerateCSFile(Application.dataPath);
                AssetDatabase.Refresh();

                excel.ConvertToScriptableObject(Application.dataPath);
                AssetDatabase.Refresh();
            }
    ...
    private static void LoadExcel()
    {
        ...
        foreach (Object obj in selection)
        {
            ...
            if (objPath.EndsWith(".xlsx", System.StringComparison.Ordinal))
            {
